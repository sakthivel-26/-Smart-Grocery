        ...
        if (!state.isInitialized) {
          ...
          setTimeout(() => {
            ...
            for (let dayOffset = 13; dayOffset >= 0; dayOffset--) {
              ...
              for (let s = 0; s < salesCount; s++) {
                ...
                for (let i = 0; i < itemCount; i++) {
                  let prodIdx: number;
                  do { prodIdx = Math.floor(Math.random() * prods.length); } while (usedIndices.has(prodIdx));
                  usedIndices.add(prodIdx);
                  const prod = prods[prodIdx];
                  const qty = Math.floor(Math.random() * 3) + 1;
                  const itemTotal = prod.sellingPrice * qty;
                  const gstAmt = prod.gstPercentage > 0 ? itemTotal * prod.gstPercentage / (100 + prod.gstPercentage) : 0;
                  items.push({
                    id: uuid(), productId: prod.id, productName: prod.name, sku: prod.sku,
                    quantity: qty, unitPrice: prod.sellingPrice, discount: 0,
                    gstPercentage: prod.gstPercentage, gstAmount: Math.round(gstAmt * 100) / 100,
                ...
                const customer = custs[Math.floor(Math.random() * custs.length)];
                const methods: PaymentMethod[] = ['cash', 'upi', 'card'];
                const method = methods[Math.floor(Math.random() * methods.length)];
                const invNum = generateInvoiceNumber(store.invoiceCounter + sampleSales.length, store.settings.invoicePrefix);

                sampleSales.push({
                  id: uuid(), invoiceNumber: invNum,
                  customerId: Math.random() > 0.4 ? customer.id : undefined,
                  customerName: Math.random() > 0.4 ? customer.name : 'Walk-in Customer',
                  items, subtotal: Math.round(subtotal * 100) / 100,
                  totalDiscount: 0, totalGst: Math.round(totalGst * 100) / 100,
                  grandTotal: Math.round(grandTotal * 100) / 100,
                  paymentMethod: method, paymentStatus: 'paid', status: 'completed',
                  cashReceived: grandTotal, changeGiven: 0, couponDiscount: 0,
                  notes: '', createdBy: 'Amit Patel', createdAt: dateStr,
                });
              }

              if (Math.random() > 0.5) {
                const expCats = ['Rent', 'Electricity', 'Salary', 'Transport', 'Supplies'];
                sampleExpenses.push({
                  id: uuid(), category: expCats[Math.floor(Math.random() * expCats.length)],
                  description: 'Regular expense', amount: Math.floor(Math.random() * 2000) + 200,
                  paymentMethod: 'cash', reference: '', date: saleDate.toISOString().split('T')[0],
                  createdBy: 'System', createdAt: dateStr,
                });
              }
            }
            ...
            set({
              ...
              invoiceCounter: store.invoiceCounter + sampleSales.length,
            });
